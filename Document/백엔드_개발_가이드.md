...
### DB 데이터 관리 및 롤백 (Data Management)
테스트 및 QA 환경에서 데이터를 초기화하거나 롤백할 때 다음 스크립트를 사용합니다. 상세 내용은 `Document/DB/DB_데이터_관리_가이드.md`를 참조하세요.
- **`npm run db:reset-qa`**: 일반 사용자 및 대여/주문 기록만 삭제 (QA 회차 전환 시).
- **`npm run db:reset-catalog`**: 물품 목록까지 초기화.
- **`npm run db:reset-full`**: 전체 공장 초기화 및 `seed.ts` 재실행.

### 파일 검증 (PDF Verification)
- 플로터 주문 시 업로드된 파일의 확장자뿐만 아니라 실제 파일 헤더(Magic Number: `%PDF-`)를 검사하여 유효한 PDF 파일인지 확인합니다.
- **참고:** 파일 업로드 API는 두 가지가 구현되어 있습니다: `POST /api/admin/upload-image` (관리자 물품 이미지, `items` 버킷)와 `POST /api/common/upload` (공용 이미지, `common` 버킷). 플로터 주문 시에는 PDF 파일이 별도로 검증/업로드됩니다.
...
## 5. 배포 정보
- 현재 백엔드는 **Railway**를 통해 배포되어 운영 중입니다. (`https://rentalweb-production.up.railway.app/api`)
- **보안 설정 (Security)**
